...
class SessionDetailsRecyclerViewAdapter(private val onItemDeletedListener: (TaskExt) -> Unit) :
    RecyclerView.Adapter<SessionDetailsRecyclerViewAdapter.SDetailsViewHolder>() {
    ...
    inner class SDetailsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val pomCountTextView: TextView =
            itemView.findViewById(R.id.taskPomodoroCountSessionDetails)
        private val deleteButton: TextView = itemView.findViewById(R.id.deleteTaskBtnSessionDetails)

        /*
        concatenate a number to string resources does not invalidate localization
         */
        @SuppressLint("SetTextI18n")
        fun bind(task: TaskExt) {
            taskNameTextView.text = task.name
            pomCountTextView.text = "${task.totalPomodoros} ${
                if (task.totalPomodoros == 1) itemView.context.resources.getString(R.string.singular_pomodoro) else itemView.context.resources.getString(
                    R.string.plural_pomodoro
                )
            }"
    ...
    inner class TaskDiffUtil(
        private val oldList: List<TaskExt>,
        private val newList: List<TaskExt>
    ) : DiffUtil.Callback() {
